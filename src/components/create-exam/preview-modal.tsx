...
interface PreviewModalProps {
  open: boolean;
  onClose: () => void;           // Modal kapatmak için fonksiyon
  title: string;                 // Quiz Title
  description: string;           // Quiz Description (Markdown)
  startDate?: string | Date;     // Form'dan aldığınız start date
...
export function PreviewModal({
  ...
  // Eğer duration string olarak geliyorsa sayıya çeviriyoruz:
  const numericDuration = duration ? Number(duration) : 120;

  return (
    <Dialog 
      open={open} 
      // "onOpenChange" ile modalın açılmasını/kapanmasını dinleriz
      onOpenChange={(val) => {
      ...
      >
        ...
        <DialogHeader className="flex items-center justify-center text-center">
          <DialogTitle className="text-md font-bold text-center">
            Here’s how your quiz start page will appear:
          </DialogTitle>
        </DialogHeader>
        ...
        >
          ...
          {/* Info Box */}
          <div className="border border-greyscale-light-200 rounded-2xl p-4 bg-white space-y-3 mb-6 md:mb-6 md:my-6 md:mx-10">
            {/* Type */}
