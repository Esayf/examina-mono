...
export const step1ValidationSchema = z.object({
  questions: z
    .array(
      z.object({
        question: z.string().min(3, { message: "Your question must be at least 3 characters. Please make it more descriptive!" }),
        correctAnswer: z.string().min(1, { message: "A correct answer must be selected." }),
        answers: z
          .array(
            z.object({
              answer: z.string().min(1, { message: "Answer option cannot be empty. Please enter some text." }).max(200),
            })
          )
          .min(2),
